<template>
  <div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :close-on-click-modal="false" width="70%"
      @close="handleClose">
      <el-form label-position="right" label-width="150px" :model="formLabelAlign" :rules="rules" ref="ruleForm"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      DeleteSingel(id) {
        http("/singleInformation/delSingleinformation", "post", {
          id: id
        }).then(
          res => {
            this.getTableData();
