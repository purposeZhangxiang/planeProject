<template>
  <div>
      ...
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="warning" plain @click="setPermission(scope.$index, scope.row)">设置权限</el-button>
    ...
    <el-dialog :title="tiTitle" :visible.sync="dialogVisible" width="30%">
      <div class="standMessage">
        ...
        <div v-if="title == 'table'" class="standMessage">
          <el-table ref="table" highlight-current-row :data="personData" class="multipleTable" tooltip-effect="dark"
            style="width: 100%;max-height:400px;overflow:auto" @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55"></el-table-column>
            <el-table-column v-for="(item,index) in personText" :key="index" :label="item.name" :prop="item.prop">
            </el-table-column>
          </el-table>
        </div>
        <!--角色编辑和新增对话框-->
    ...
    <el-dialog :title="tiTitle" :visible.sync="treeVisible" width="30%">
      <el-tree :data="dataTree" ref="treeForm" :check-strictly="true" :highlight-current="true"
        @check-change="handleClick" show-checkbox node-key="id" :props="defaultProps"
        :default-checked-keys="dateTreeSeleect"></el-tree>
      <span slot="footer" class="dialog-footer">
